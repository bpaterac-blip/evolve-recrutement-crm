...
/** Raisons affichées dans Tous les profils (tableau) — distinctes du pipeline ChuteModal */
const CHUTE_TABLE_RAISONS = [
  'Pas disponible',
  'Trop loin',
  'Salaire insuffisant',
  "Pas intéressé par l'indépendance",
  'Déjà indépendant',
  'Autre',
]

export default function ChuteRaisonModal({ profile, onClose, onSaved }) {
  ...
  return (
    <div style={{ background: '#F5F0E8', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', minWidth: 400, maxWidth: 480, overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
      <div style={{ background: ACCENT, padding: '20px 24px' }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 600, color: 'white' }}>Raison de la chute</div>
        <div style={{ fontSize: 13, color: GOLD, marginTop: 4 }}>{profileName} · Stade actuel : {stageLabel}</div>
      </div>
      <div style={{ padding: 24 }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <select value={raison} onChange={(e) => setRaison(e.target.value)} required style={{ width: '100%', padding: '10px 12px', fontSize: 13, border: '1px solid #E5E0D8', borderRadius: 6 }}>
            ...
            {CHUTE_TABLE_RAISONS.map((t) => (
              <option key={t} value={t}>{t}</option>
            ))}
          </select>
        </div>
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: '#666', marginBottom: 6 }}>Précision (optionnel)</label>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          ...
          <button type="button" onClick={handleSave} disabled={saving || !raison} style={{ padding: '10px 16px', fontSize: 13, border: 'none', borderRadius: 6, background: GOLD, color: ACCENT, cursor: 'pointer', fontWeight: 600, opacity: saving || !raison ? 0.6 : 1 }}>Confirmer</button>
        </div>
      </div>
    </div>
  )
