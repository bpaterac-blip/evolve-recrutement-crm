    ...
    <div className="ov fixed inset-0 bg-black/35 z-[200] flex items-start justify-end" onClick={onClose}>
      <div className="modal bg-[var(--surface)] w-[540px] h-screen overflow-y-auto shadow-[-20px_0_60px_rgba(0,0,0,0.15)] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="mhd py-4 px-5 border-b border-[var(--border)] flex items-center justify-between sticky top-0 bg-[var(--surface)] z-10 shrink-0">
          ...
          <button type="button" className="text-lg cursor-pointer text-[var(--t3)] bg-none border-none leading-none flex items-center justify-center" onClick={onClose}><IconClose /></button>
        </div>
        <div className="mbd p-5 flex-1">
          ...
          <div className="dr flex items-start gap-2.5 mb-2">
            ...
            {scoreEdit ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" className="text-xs text-[var(--t3)] hover:text-[var(--accent)] flex items-center gap-1" onClick={() => { setScoreEdit(true); setNewScore(String(profile.sc)); }}><IconPencil /> Corriger</button>
              </div>
            )}
          </div>
          <div className="dr flex items-start gap-2.5 mb-2 relative">
          ...
          <button type="button" className="btn bo bsm mt-1.5 mb-4 flex items-center gap-2" onClick={() => saveNote(profile.id, note)}><IconSave /> Enregistrer la note</button>
          ...
          {profile.stg !== 'Recruté' && (
            <>
              ...
              <button type="button" className="ab w-full py-2 px-3 rounded-lg border border-[var(--b2)] bg-[var(--s2)] text-[13px] cursor-pointer text-left mb-1.5 flex items-center gap-2 hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] transition-all" onClick={() => {}}><IconCalendar /> Planifier un RDV Google Meet</button>
              {STAGES[STAGES.indexOf(profile.stg) + 1] && <button type="button" className="ab w-full py-2 px-3 rounded-lg border border-[var(--b2)] bg-[var(--s2)] text-[13px] cursor-pointer text-left mb-1.5 flex items-center gap-2 hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] transition-all" onClick={() => changeStage(profile.id, STAGES[STAGES.indexOf(profile.stg) + 1])}><IconArrowUp /> Avancer vers {STAGES[STAGES.indexOf(profile.stg) + 1]}</button>}
            </>
          )}
          ...
          <div>
            {acts.map((a, i) => (
              <div key={i} className={`ai2 flex gap-3 py-2.5 border-b border-[var(--border)] text-[13px] last:border-b-0 ${a.type === 'lem' ? 'lem-ev bg-[var(--lbg)] mx-[-4px] px-1 py-2.5 rounded-md border-none' : ''} ${a.type === 'stg' ? 'stg-ev bg-[#EDF4FF] mx-[-4px] px-1 py-2.5 rounded-md border-none' : ''} ${a.type === 'mat' ? 'mat-ev bg-[#F5F0FA] mx-[-4px] px-1 py-2.5 rounded-md border-none' : ''}`}>
                ...
                <div><div className="at font-medium mb-0.5 flex items-center gap-1.5"><span className="act-ico text-[13px] inline-flex"><ActivityIcon ico={a.ico} /></span>{a.t}</div><div className="an text-[var(--t2)] text-[12.5px] leading-snug">{a.n}</div></div>
              </div>
            ))}
